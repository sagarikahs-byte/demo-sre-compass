...
On the sample data, 40 alerts become **one primary heading** — `errorCode 503 + /api/checkout`, likely caused by a recent `payments-service` deploy (a checkout dependency) — with the gateway 5xx flagged as an echo and the rest as low-signal.
